const autoUpdate = (name, repo, branch) => {
    ...
    Events.on(ClientLoadEvent, () => {
        if(!Core.settings.getBool("mod." + name + ".autoupdate-disabled")){
            Core.net.httpGet(
                "https://raw.githubusercontent.com/" + repo + "/" + branch + "/mod.hjson",
                (result) => {
                    var modHjson = JSON.parse(result.getResultAsString());
                    var comparable = (number) => Number(number.toString().replace(".", ""));
                    if(comparable(modLocal.version) < comparable(modHjson.version)){
